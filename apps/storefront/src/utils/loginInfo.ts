...
export const loginInfo = async () => {
  // Use Stencil-provided token if available (recommended for Stencil integration)
  const stencilToken = window.STOREFRONT_GRAPHQL_API_TOKEN;
  
  if (stencilToken) {
    console.log('✅ Using Stencil-provided GraphQL token');
    store.dispatch(setBcGraphQLToken(stencilToken));
    return;
  }
  
  // Fallback: Generate token via B2B API (for headless or when Stencil token not available)
  //console.log('⚠️ No Stencil token found, generating via B2B API...');
